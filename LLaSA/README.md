# 🌋 LLaSA: Large Language and Sensor Assistant
...
Please run `scripts/pretain_llasa.sh` and then `scripts/finetune_llasa.sh`. Alternate hyperparameters can be used with `*_variable.sh` files. Example slurm-based hyperparameter tuning scripts are included as `hyperparameter_tune_sbatch.sh`. 

We are also attaching the original LLaVA training procedures for additional insight.

## Original Guidelines for LLaVA Training Procedure
